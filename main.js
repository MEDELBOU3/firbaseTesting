    /**
      ...
      const AuraStreamApp = {
        ...
        initializeFirebase: function() {
            ...
            const appObject = AuraStreamApp; // Use a consistent reference

            utils.log(">>> ENTERING initializeFirebase function.");

            // --- Config and SDK Checks (Keep these robust) ---
            if (!config?.apiKey || config.apiKey.startsWith('YOUR_') || !config.authDomain || !config.projectId) {
                utils.error("Firebase config missing/incomplete/placeholder.");
                alert("CRITICAL: Firebase not configured. Auth disabled.");
                return false;
            }
            ...
            if (typeof firebase === 'undefined' || !firebase.app || !firebase.auth || !firebase.storage || !firebase.firestore) {
                 utils.error("Required Firebase SDKs not fully loaded.");
                 alert("CRITICAL: Firebase libraries missing. Core features disabled.");
                 return false;
             }
             // --- End Checks ---
            ...
            try {
                ...
                if (!state.firebaseApp) {
                    ...
                    try { state.firebaseStorage = firebase.storage(); } catch (e) { utils.error("Storage init failed:", e); state.firebaseStorage = null; }
                    try { state.firebaseFirestore = firebase.firestore(); } catch (e) { utils.error("Firestore init failed:", e); state.firebaseFirestore = null; }
                    utils.log("Firebase services initialized.");
                } else {
                     utils.log("Firebase services already initialized.");
                 }

                // --- Setup Auth State Listener ---
                if (!state.authListenerUnsubscribe && state.firebaseAuth) {
                    utils.log(">>> Attaching onAuthStateChanged listener NOW.");
                    state.authListenerUnsubscribe = state.firebaseAuth.onAuthStateChanged(user => {
                        // *** THIS IS THE CRITICAL PART FOR PAGE LOAD ***
                         utils.log(`>>> onAuthStateChanged FIRED! Timestamp: ${Date.now()}`);
                         utils.log(`   User object received:`, user ? `UID: ${user.uid}, Email: ${user.email}` : 'null');

                         let isInitialAuthCheck = !state.initializationComplete; // Track first run
                         utils.log(`   Is Initial Auth Check (after core init)?: ${isInitialAuthCheck}`);

                        // ** Update State BASED ON 'user' **
                        if (user) {
                             utils.log(`   --> Setting state to LOGGED IN.`);
                             ...
                             utils.log(`   Current User State Updated:`, JSON.parse(JSON.stringify(state.currentUser || {}))); // Deep copy for logging
                        } else {
                             utils.log(`   --> Setting state to LOGGED OUT.`);
                             state.isLoggedIn = false;
                             state.currentUser = null;
                         }
                         utils.log(`   State AFTER update: isLoggedIn=${state.isLoggedIn}`);

                        // ** Update UI based on the NEW state **
                         utils.log(`   Calling updateAuthUI() to reflect state...`);
                         appObject.modules.auth.updateAuthUI(); // <<< ENSURE this runs AFTER state is set
                         utils.log(`   Finished updateAuthUI() call.`);


                         // ** Trigger Async Load/Redirects ONLY ONCE (Initial Check) **
                         if (isInitialAuthCheck) {
                             utils.log(`   Processing Initial Auth Check actions...`);
                             if (state.apiKeyValid) {
                                 utils.log("      TMDB key valid -> Running async TMDB data fetches...");
                                 appObject.runAsyncInits(); // Load movies, etc.
                             } else {
                                 utils.warn("      TMDB key invalid - Skipping initial data fetch.");
                                  if (state.introHidden) appObject.displayDataLoadingErrors("Invalid API Key"); // Show error if intro done
                             }
                              // Mark overall app initialization (including first auth check) as done
                              state.initializationComplete = true;
                              utils.log("   <<< Core Initialization sequence NOW fully complete.");
                         } else {
                              utils.log(`   Not initial check, skipping async init triggers.`);
                         }

                        // ** Post-Load Redirect (If just logged in/signed up VIA MODAL) **
                        if (user && !isInitialAuthCheck && appObject.elements.launchAppModal?.classList.contains('show')) {
                             utils.log(`   Post-load auth change + Modal Open detected! Closing modal and Redirecting...`);
                             appObject.modules.modals?.closeAuthModal();
                             setTimeout(() => { window.location.href = appObject.config.AUTH_REDIRECT_URL; }, 150);
                             return; // Exit callback because we're navigating away
                         }

                         // ** Ensure Intro Hidden ** (Handles race condition if auth faster than intro)
                         if (!state.introHidden && appObject.modules?.introVideo?.hideOverlay) {
                              utils.log(`   Auth resolved before intro hidden. Forcing hideOverlay(true).`);
                             appObject.modules.introVideo.hideOverlay(true);
                         }

                        utils.log(`<<< Exiting onAuthStateChanged callback for this cycle.`);

                    }); // End of onAuthStateChanged callback
                    utils.log(">>> onAuthStateChanged listener ATTACHED.");

                } else if (state.authListenerUnsubscribe) {
                     utils.log("Listener already attached.");
                } else if (!state.firebaseAuth) {
                     utils.error("Cannot attach auth listener - Firebase Auth service not initialized!");
                     return false; // Critical failure if auth didn't init
                }

                utils.log("<<< Exiting initializeFirebase function (Success).");
                return true;

            } catch (error) {
                 utils.error("CRITICAL error during Firebase Initialization:", error);
                alert("Firebase setup failed. Authentication unavailable.");
                 if (appObject.modules?.auth?.updateAuthUI) appObject.modules.auth.updateAuthUI(); // Try to show logged out
                return false;
            }
            ...
            updateDisplay: function(bestTrailer = null) {
             ...
             displayError: function(message = "Cannot load trailer.") {
              ...
              auth: {
                ...
                const utils = AuraStreamApp.utils;
                const els = AuraStreamApp.elements;
                const state = AuraStreamApp.state; // Get fresh state

                // *** ADD LOGS AT THE START ***
                utils.log(`>>> ENTERING updateAuthUI function. Timestamp: ${Date.now()}`);
                utils.log(`   State passed to updateAuthUI: isLoggedIn=${state.isLoggedIn}`);
                 utils.log(`   User Data in state:`, state.currentUser ? JSON.parse(JSON.stringify(state.currentUser)) : 'null');


                // Essential element check (Keep this)
                if (!els.loginSignupButton || !els.userInfoArea || !els.logoutButton ||
                    !els.userAvatar || !els.userAvatarInitials || !els.userAvatarImage || !els.userDisplayName) {
                    if(AuraStreamApp.config.DEBUG_MODE) utils.warn(`updateAuthUI: Skipping update, essential header elements missing.`);
                    return;
                }

                // --- Main Logic (Should be correct, but logs help confirm) ---
                if (state.isLoggedIn && state.currentUser) {
                     utils.log(`   --> Setting UI to LOGGED IN for: ${state.currentUser.displayName || state.currentUser.email}`);
                    ...
                    if (photoURL) {
                         utils.log(`      Displaying AVATAR IMAGE from URL: ${photoURL}`);
                        els.userAvatarImage.src = photoURL;
                        els.userAvatarImage.alt = `${displayName}'s profile picture`;
                        els.userAvatarImage.classList.remove('d-none');
                ...
                utils.log(`<<< Exiting updateAuthUI function.`);
               }, 
